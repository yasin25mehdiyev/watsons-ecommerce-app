<template>
  <section class="py-8 md:py-12 bg-watsons-titan">
    <div class="max-w-7xl mx-auto px-4">
      <div class="flex flex-col md:flex-row items-start md:items-center md:justify-between gap-4 md:gap-6">

        <div class="text-left">
          <h3 class="text-xl md:text-2xl font-bold text-watsons-coal mb-2 md:mb-4">
            Sign up to our newsletter
          </h3>
          <p class="text-[#2A2A4899] text-sm md:text-base">
            Be the first to hear about the latest offers.
          </p>
        </div>

        <div class="flex flex-row gap-3 w-full md:w-auto">
          <input type="email" placeholder="Your email address" class="w-full md:w-[412px] px-4 py-3 border border-gray-300 rounded-lg 
                   focus:outline-none focus:border-watsons-teal text-sm" />

          <button class="bg-watsons-pink text-white px-6 py-3 rounded-lg font-medium 
                   hover:bg-pink-600 transition-colors whitespace-nowrap text-sm md:text-base">
            SUBSCRIBE
          </button>
        </div>

      </div>
    </div>
  </section>
</template>
